// src/domains/memo/MemoFormatBar.jsx
...
import {
  ...
  HeadingToggleIcon, WidthFitIcon, TrashIcon, LinkIcon, TemplateIcon, TemplateSaveIcon
...
const MemoFormatBar = ({
  ...
  // ★ 픽스 1: 일일이 지울 필요 없는 가짜 플레이스홀더 CSS를 글로벌에 조용히 주입합니다.
  useEffect(() => {
    if (!document.getElementById('fold-placeholder-styles')) {
      const style = document.createElement('style');
      style.id = 'fold-placeholder-styles';
      style.innerHTML = `
        .fold-title:empty::before, .fold-content:empty::before {
  ...
  // ★ 픽스: svgPlay 아이콘에 달려있던 중복 onclick 이벤트를 삭제하여 더블 토글 먹통 버그 원천 해결
  const foldHTML = `<div style="position:relative; margin:15px 0; padding-top:15px;" contenteditable="false"><button type="button" onclick="this.parentElement.remove()" style="position:absolute; top:0; right:0; background:#e53e3e; color:white; border:none; border-radius:4px; padding:4px 8px; cursor:pointer; font-size:11px; font-weight:bold; z-index:10; display:flex; align-items:center; gap:4px;">${svgClose} 박스 삭제</button><details open style="border: 1px solid var(--border-color); border-radius: 8px; background: var(--table-bg-alt); overflow: hidden; font-size:13px;"><summary onclick="if(event.target.closest('.fold-title')) event.preventDefault();" style="padding: 10px 15px; font-weight: 900; cursor: default; color: var(--primary-color); outline: none; list-style:none; display:flex; align-items:center; gap:8px;"><span contenteditable="false" style="display:flex; align-items:center; justify-content:center; cursor:pointer; user-select:none; padding:2px;">${svgPlay}</span><span class="fold-title" contenteditable="true" data-placeholder="접기 박스 제목 (Tab을 눌러 내용으로)" style="outline:none; flex:1; min-width:50px; cursor:text;"></span></summary><div class="fold-content" contenteditable="true" data-placeholder="숨길 내용을 입력하세요..." style="padding: 15px; border-top: 1px dashed var(--border-color); line-height: 1.6; background: var(--surface-color); outline:none; min-height:50px;"></div></details></div><div><br></div>`;
