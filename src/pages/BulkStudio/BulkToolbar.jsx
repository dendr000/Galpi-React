...
const BulkToolbar = ({ 
  works, selectedWorkId, loadCharacters, addRow, addColumn, 
  tabMode, setTabMode, sortTable, applyBatchValue, setFindReplaceModal,
  ...
  const handleAddRow = () => {
    console.log(`[BulkToolbar] 행 추가 트리거 가동`);
    addRow(Number(addCount) || 1);
    setAddCount(1);
  };

  const handleAddCol = () => {
    console.log(`[BulkToolbar] 열 추가 트리거 가동`);
    ...
    <>
      <div className="header">
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          ...
          <h2 style={{ margin: 0, color: 'var(--primary-color)', fontSize: '20px' }}>📦 캐릭터 일괄 관리 스튜디오</h2>
        </div>
      ...
      <div className="toolbar">
        <div className="toolbar-group">
          <input type="number" value={addCount} min="1" onChange={e => setAddCount(e.target.value)} className="tb-input-sm" title="추가할 행 개수" style={{ width: '40px' }} />
        ...
        <div className="toolbar-group">
          <button className="icon-btn" onClick={() => { console.log("[BulkToolbar] 탭 모드 스위칭"); setTabMode(prev => prev === 'horizontal' ? 'vertical' : 'horizontal'); }} title={`탭 이동 방향: ${tabMode === 'horizontal' ? '가로(좌우)' : '세로(상하)'}`} style={tabMode === 'vertical' ? { color: 'var(--primary-color)', background: 'rgba(59,91,219,0.1)', borderColor: 'var(--primary-color)' } : {}}>
            {tabMode === 'horizontal' ? (
          ...
          <button className="icon-btn" onClick={() => { console.log("[BulkToolbar] 찾아바꾸기 모달 개방"); setFindReplaceModal({ isOpen: true }); }} title="찾아 바꾸기 (치환)">
        ...
        <div className="toolbar-group">
          ...
          <select className="tb-select" onChange={e => sortTable(e.target.value)}>
            <option value="card">🗂️ 카드 배치 순</option>
        ...
        <div className="toolbar-group batch-group" style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
          <span className="tb-label" style={{ color: 'var(--primary-color)', fontWeight: 'bold' }}>✔️ 선택 일괄 적용:</span>
      ...
      <div className="bulk-toolbar" style={{ borderBottom: '1px solid var(--border-color)', padding: '8px 20px', background: 'var(--surface-color)', display: 'flex', gap: '15px', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
          ...
          <input type="text" className="tb-input" value={labels.label1} onChange={e => { console.log("[BulkToolbar] 라벨1 변경"); setLabels(prev => ({ ...prev, label1: e.target.value })); }} style={{ padding: '4px 8px', width: '100px' }} />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
          <label style={{ fontSize: '12px' }}>이름 아래:</label>
          <input type="text" className="tb-input" value={labels.label2} onChange={e => { console.log("[BulkToolbar] 라벨2 변경"); setLabels(prev => ({ ...prev, label2: e.target.value })); }} style={{ padding: '4px 8px', width: '140px' }} />
        </div>
      </div>
    </>
  );
